...
class Autoencoder():
    ...
    def _build(self):
        ...
        shape_before_flattening = K.int_shape(x)[1:] # shape[0] is batch size

        x = Flatten()(x)
        # Dense(units == ouput dimension
        encoder_output = Dense(units=self.z_dim, name='encoder_output')(x)
        # print(encoder_output.shape) (?, self.z_dim)

        self.encoder = Model(encoder_input, encoder_output)

        # THE DECODER
        # shape=(self.z_dim,) 이렇게 output dim을 생략하면 뒤 레이어의 input에
        # 맞게 채워진다.
    ...
    def train(self, x_train, batch_size, epochs, run_folder, print_every_n_batches=100,
        ...
        self.model.fit(
            x_train,
            x_train, # Input == Output in AutoEncoder
